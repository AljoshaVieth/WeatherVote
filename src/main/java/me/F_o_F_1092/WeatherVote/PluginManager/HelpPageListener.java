...
public class HelpPageListener {
  ...
  protected static String getNavBar(
      List<HelpMessage> personalHelpMessages,
      List<HelpMessage> personalHelpPageMessages,
      int page) {
    List<JSONMessage> jsonMessages = new ArrayList<JSONMessage>();

    JSONMessage stringStartMessage = new JSONMessage(Options.msg.get("color.1") + "Åòm-");

    JSONMessage stringLastPageMessage1;
    JSONMessage stringLastPageMessage2;
    if (page != 0) {
      stringLastPageMessage1 =
          new JSONMessage(
      ...
      stringLastPageMessage1 = new JSONMessage(Options.msg.get("color.1") + "Åòm------");
      stringLastPageMessage2 = new JSONMessage("");
    }

    JSONMessage stringHeaderMessage =
        new JSONMessage(
            Options.msg.get("color.1")
                + "Åòm---------Åòr "
                + pluginNametag.replace("Åòl", "")
                + Options.msg.get("color.1")
                + "Åòm---------");
    ...
    if ((page + 1) < getMaxPlayerPages(personalHelpMessages)) {
      ...
      stringNextPageMessage1 = new JSONMessage(Options.msg.get("color.1") + "Åòm------");
      stringNextPageMessage2 = new JSONMessage("");
    }

    JSONMessage stringEndMessage = new JSONMessage(Options.msg.get("color.1") + "Åòm-");
  ...
  public static int getMaxPlayerPages(List<HelpMessage> personalHelpMessages) {
    return (int)
        java.lang.Math.ceil(((double) personalHelpMessages.size() / (double) maxHelpMessages));
  }
}
